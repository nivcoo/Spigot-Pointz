...
public class Commands implements CommandExecutor {
	...
	/**
	 * public static int getMoneyPlayer(Player player) throws SQLException { Pointz
	 * pointz = Pointz.get(); return pointz.getWebsiteAPI().getMoneyPlayer(player);
	 *
	 * }
	 **/

	/**
	 * public static HashMap<String, String> getPlayerInfos(Player player) throws
	 * SQLException { Pointz pointz = Pointz.get(); return
	 * pointz.getWebsiteAPI().getPlayerInfos(player);
	 * 
	 * }
	 **/
	 ...
	 * Connection c = null; PreparedStatement ps = null; ResultSet rs = null; try {
	 * c = bdd.getConnection(); ps = c.prepareStatement("SELECT pseudo FROM users
	 * WHERE pseudo = ?");
	 * 
	 * ps.setString(1, player.getName()); rs = ps.executeQuery(); if (rs.next()) {
	 * return rs.getString("pseudo"); }
	 * 
	 * } catch (SQLException e) { e.printStackTrace(); } finally { this.close(c, ps,
	 * rs); } return null;
	 * 
	 * }
	 **/
	 ...
	 * bdd.getConnection(); ps = c.prepareStatement("UPDATE users SET money = ?
	 * WHERE pseudo = ?");
	 * 
	 * ps.setInt(1, money); ps.setString(2, player.getName()); ps.executeUpdate(); }
	 * catch (SQLException e) { e.printStackTrace(); } finally { this.close(c, ps,
	 * null); }
	 * 
	 * }
	 * 
	 * private void close(Connection c, PreparedStatement ps, ResultSet rs) { try {
	 * if (c != null) c.close(); if (ps != null) ps.close(); if (rs != null)
	 * rs.close(); } catch (Exception e) { System.out.println("Error while closing
	 * database c: " + e); } }
	 **/
